...
useMemo – оптимізація для даних.  useCallback – оптимізація для функцій.`},{id:8,link:"https://www.youtube.com/watch?v=hkrmyIecHR0&ab_channel=UlbiTV",title:"Реалізація debounce [ 24:32 ]",requirements:["Написати функцію debounce(callback, delay), яка повертає нову функцію.","Якщо викликають новий раз раніше, ніж пройшов delay, попередній таймер скасовується.","Після затримки викликається callback з останніми аргументами.","5 раз визивається fetching() з різними аргументами а ми вивели лише fetching 5"],starterCode:`const fetchUrl = url => {
...
fetching(3);
fetching(4);
fetching(5);`,solution:`const fetchUrl = url => {
...
fetching(3);
fetching(4);
fetching(5);`,description:` debounce - для того, щоб приривати попередні дії і відкладено виконувати останні.
    Кожен виклик скасовує попередній таймер. Після останнього виклику fetching(5) таймер
     ...
     фронтенд-розробці, особливо при роботі з подіями, які можуть спрацьовувати дуже часто.
      Його головна мета — зменшити кількість викликів функції, що зберігає ресурси та покращує
       ...
       системи або сервера.`},{id:9,link:"https://www.youtube.com/watch?v=hkrmyIecHR0&ab_channel=UlbiTV",title:"Реалізація debounce [ 26:25 ]",requirements:["Щоб крім пятірки, ще появлявся Bob "],starterCode:`const fetchUrl = url => {
...
fetching(3);
fetching(4);
fetching(5);`,solution:`const fetchUrl2 = function (url) {
...
fetching2(2);
fetching2(3);
fetching2(4);`,description:`Щоб ще виводився Bob (поміняв стрілку на function + bind(user)). 
...
оскільки this вже зафіксований першим bind. ( Запамятовує перший контекст.)`},{id:10,link:"https://www.youtube.com/watch?v=hkrmyIecHR0&ab_channel=UlbiTV",title:"Деревовидна структура [ 28:38 ]",requirements:["Пройтися по всій структурі та зібрати values в масив.","Використати ітеративний підхід (через стек)"],starterCode:`const tree = {
  value: 1,
  children: [
    {
      value: 2,
...
console.log(getTreeValues(tree)); // [1,4,6,5,2,3]`,description:`Якщо велика вкладеність, то краще через стек, щоб уникнути помилки
     переповнення.`},{id:11,link:"",title:"Деревовидна структура: Варіант 2",requirements:["Пройтися по всій структурі та зібрати values в масив.","Використати рекурсивний підхід"],starterCode:`const tree = {
  value: 1,
  children: [
    {
      value: 2,
...
Повертаємо масив. Рекурсія в JS обмежена ~10000 викликів.
 ...
 рекурсія може викликати RangeError: Maximum call stack size exceeded`},{id:12,link:"https://www.youtube.com/watch?v=hkrmyIecHR0&ab_channel=UlbiTV",title:"Рефокторинг коду: React. [32:30]",requirements:["Зробити ревю та виправити помилки в коді нижче."],starterCode:`
...
`,description:""},{id:14,link:"https://www.youtube.com/watch?v=nqwJDi-z738",title:"На синхронність JavaScript  [ 41:00 ]",requirements:["В якій послідовності виведуться console.log"],starterCode:`console.log('0');
...
function range(arr) {
 ...
 console.log(range([1, 4, 3, 2])); // 1-4`,description:""},{id:16,link:"https://www.youtube.com/watch?v=nqwJDi-z738",title:"Розкласти масив на рядок з діапазонами [42:57]",requirements:["Варіант 2"],starterCode:`range([1, 4, 5, 2, 3, 9, 8, 11, 0]);
range([1, 4, 3, 2]);
...
console.log(sum); //26`,description:""},{id:124,link:"",title:" ",requirements:["Створення "],starterCode:"",solution:"",description:""},{id:126,link:"",title:" ",requirements:["Створення "],starterCode:"",solution:"",description:""},{id:128,link:"",title:" ",requirements:["Створення "],starterCode:"",solution:"",description:""}],d=[{id:1,title:"Асинхронне програмування ",requirements:["досягається шляхом відкладених викликів ф-цій ","де ініціалізація асинхронної операції і обробка її результату","це різні дії."]},{id:2,title:"API інтерфейс прикладного програмування)",requirements:["— набір чітко визначених правил звязку між","різними програмними компонентами."]},{id:3,title:"AJAX (Asynchronus JavaScript and XL)",requirements:["метод отримання або відправлення даних","з подальшим оновленням інтерфейсу за цими даними,","без потреби перезавантаження сторінки."]},{id:4,title:"CORS (Cross Origin Resource Sharing)",requirements:["- де Origin це і є домен, порт або протокол","-  спільне використання ресурсів між спільними джерелами.",""]},{id:5,title:"Пагінація",requirements:["— це прийом, за яким, на перший і кожен наступний","Get-запит повертається не вся колекція,","а певна її частина."]},{id:6,title:"http протокол",requirements:["— набір правил і домовленостей, що використовуються","для передачі даних у мережі.",""]},{id:7,title:"REST (representational state trancfer)",requirements:["— стиль бекенд-архітектури, грунтується на наборі принципів","які описують, яким чином визначаються і адресуються мереживі ресурси.",""]},{id:8,title:"SOLID",requirements:["— набір принципів об’єктно-орієнтованого програмування,","які допомагають створювати гнучкий, підтримуваний","та масштабований код.","Складається з п’яти основних принципів:","S — Single Responsibility Principle","Принцип єдиної відповідальності.","Клас або модуль повинен виконувати лише одну задачу","і мати тільки одну причину для зміни.","","O — Open/Closed Principle","Принцип відкритості/закритості.","Програмні сутності повинні бути відкритими для","розширення, але закритими для модифікації.","","L — Liskov Substitution Principle","Принцип підстановки Лісков.","Дочірні класи повинні мати можливість","заміняти батьківські без порушення логіки програми.","","I — Interface Segregation Principle","Принцип розділення інтерфейсів.","Не слід змушувати класи реалізовувати методи,","які їм не потрібні.","","D — Dependency Inversion Principle","Принцип інверсії залежностей.","Модулі верхнього рівня не повинні залежати","від модулів нижнього рівня.","Обидва повинні залежати від абстракцій."]},{id:99,title:"",requirements:["","",""]}],g="_title_f5mrh_1",p={title:g},r={jsTasks:m,itBasics:d},v=()=>{const[o,i]=c.useState(Object.keys(r)[0]),s=e=>{i(e.target.value)},n=o?r[o]:[];return t.jsxs("main",{style:{padding:"4px"},children:[t.jsx("h2",{className:p.title,children:"Задачі JS"}),t.jsx(a,{value:o,onChange:s,options:Object.keys(r)}),n.length===0&&o&&t.jsx("p",{children:"Немає задач для цієї категорії."}),n.sort((e,l)=>e.id-l.id).map(e=>t.jsx(u,{task:e},e.id))]})};export{v as default};
